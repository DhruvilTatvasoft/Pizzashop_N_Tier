...
public class TaxesRepository : ITaxesRepository
{
    ...
    public bool addNewTax(Taxesandfee tax)
    {
        ...
        {
            // Taxesandfee taxxx = _context.Taxesandfees.FirstOrDefault(t=>t.Taxname.ToLower().Trim() == tax.Taxname.ToLower().Trim() && t.Taxid!=tax.Taxid && t.Isdeleted != true);
            if( _context.Taxesandfees.FirstOrDefault(t=>t.Taxname.ToLower().Trim() == tax.Taxname.ToLower().Trim() && t.Taxid!=tax.Taxid && t.Isdeleted != true) != null){
                return false;
            }
            else{
